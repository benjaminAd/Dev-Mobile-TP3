...
class Planning : AppCompatActivity() {
    ...
    private val mPlanningViewModel: PlanningViewModel2 by viewModels {
        PlanningViewModelFactory((application as PlanningApplication).repository)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = PlanningListAdapter()
        recyclerView.adapter = adapter
        recyclerView.layoutManager = LinearLayoutManager(this)

        mPlanningViewModel.readAllData.observe(this, { plannings ->
            plannings.let { adapter.submitList(it) }
        })
